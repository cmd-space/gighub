...
//			$profile = null;
	...
	/**
	 * gets profiles by profile user name
	 *
	 * @param \PDO $pdo PDO connection object
	 * @param string $profileUserName user name content to search for
	 * @return \SplFixedArray SplFixedArray of Profiles found
	 * @throws \PDOException when mySQL related errors occur
	 * @throws \TypeError when variables are not the correct data type
	 */
	public static function getProfileByProfileUserName(\PDO $pdo, string $profileUserName) {
		...
		if(empty($profileUserName) === true) {
			throw(new \PDOException("profile user name is invalid, please come again"));
		}

		// create query template
		$query = "SELECT profileId, profileOAuthId, profileTypeId, profileBio, profileImageCloudinaryId, profileLocation, profileOAuthToken, profileSoundCloudUser, profileUserName FROM profile WHERE profileUserName LIKE :profileUserName";
		$statement = $pdo->prepare($query);
		...
		// build an array of profiles, if more than one
		$profiles = new \SplFixedArray($statement->rowCount());
		$statement->setFetchMode(\PDO::FETCH_ASSOC);
		while(($row = $statement->fetch()) !== false) {
			try {
	...
	public static function getProfileByProfileId(\PDO $pdo, int $profileId) {
		...
		if($profileId <= 0) {
			throw(new \PDOException("profile id is 0 or negative"));
		}

		// create query template
		$query = "SELECT profileId, profileOAuthId, profileTypeId, profileBio, profileImageCloudinaryId, profileLocation, profileOAuthToken, profileSoundCloudUser, profileUserName FROM profile WHERE profileId = :profileId";
		$statement = $pdo->prepare($query);
